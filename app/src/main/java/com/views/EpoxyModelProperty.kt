...
class EpoxyModelProperty<T>(private val force: Boolean = false, default: () -> T) : ReadWriteProperty<EpoxyController, T> {
    ...
    override fun setValue(thisRef: EpoxyController, property: KProperty<*>, value: T) {
        if (this.value != value) {
            this.value = value
            if (force && thisRef is PagedListEpoxyController<*>) thisRef.requestForcedModelBuild() else thisRef.requestModelBuild()
        }
    }
}